...
CREATE TABLE IF NOT EXISTS `type_entries_subcategories` (
    `ID` INT NOT NULL AUTO_INCREMENT,
    `NAME` VARCHAR(255) NOT NULL,
    `CATEGORY_ID` INT NOT NULL,
    PRIMARY KEY (`ID`),
    FOREIGN KEY (`CATEGORY_ID`) REFERENCES `type_entries_categories`(`ID`) ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS `entries` (
    ...
    FOREIGN KEY (`ID_SUBCATEGORY_TYPE`) REFERENCES `type_entries_subcategories`(`ID`) ON DELETE CASCADE
);
